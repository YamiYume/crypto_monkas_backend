...
rsa_enc_parser = utils.enc_parser_keyless()
rsa_dec_parser = reqparse.RequestParser()
rsa_dec_parser.add_argument(
    "ciphertext",
    type=str,
    required=True,
...
class RSAEnc(Resource):
    def post(self):
        args = rsa_enc_parser.parse_args()
        ...
        a = t
        while a >= t or t % a == 0:
            a = lazyutils.generate_prime_number()
        ciphertext = lazyutils.encRSA(plaintext, a, n)
        with open(utils.FILEPATH + "rsa_ciphertext.txt", "w") as f:
            for value in ciphertext:
                f.write(f"{value}\n")
        with open(utils.FILEPATH + "rsa_key.txt", "w") as f:
            for value in (p, q, a):
                f.write(f"{value}\n")
        with open(utils.FILEPATH + "rsa_ciphertext.txt", "rb") as ciphertext:
            ciphertext = base64.b64encode(ciphertext.read()).decode()
        with open(utils.FILEPATH + "rsa_key.txt", "rb") as key:
            key = base64.b64encode(key.read()).decode()
        return {"ciphertext": ciphertext, "key": key}


class RSADec(Resource):
    def post(self):
        args = rsa_dec_parser.parse_args()
        ...
        p, q, a = key
        plaintext = lazyutils.decRSA(ciphertext, a, p, q)
